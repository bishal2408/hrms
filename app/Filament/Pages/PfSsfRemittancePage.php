 ...
 * Mirrors VatRegisterPage exactly (same page shape, same
 * payroll_accountant-owns-compliance-reports access rule).
 */
class PfSsfRemittancePage extends Page implements HasSchemas
{
    use InteractsWithSchemas;

    protected string $view = 'filament.pages.pf-ssf-remittance';

    protected static ?string $title = 'PF/SSF Remittance';

    protected static ?string $slug = 'payroll/pf-ssf-remittance';

    protected static string|UnitEnum|null $navigationGroup = 'Payroll';

    protected static string|BackedEnum|null $navigationIcon = Heroicon::OutlinedBuildingLibrary;

    protected static ?string $navigationLabel = 'PF/SSF Remittance';
    ...
    public function getReportProperty(): PfSsfRemittanceReport
    {
        [$from, $until] = $this->resolveDateRange();

        return app(PayrollComplianceReportService::class)->pfSsfRemittance($from, $until);
    }

    /** The currently-selected range, as AD query params — PfSsfRemittanceExportController reads these directly, no BS parsing happens there. */
    public function getExportUrlProperty(): string
    {
        [$from, $until] = $this->resolveDateRange();

        return route('pf-ssf-remittance.export', array_filter([
